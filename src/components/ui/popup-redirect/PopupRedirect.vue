<template>
  <div v-if="isVisible" class="popup-wrapper">
    ...
    <div class="card-request-game popup flex flex-col md:flex-row text-center md:text-start gap-4 items-center rounded-md p-4 bg-nj-card my-10 mt-0 mx-2">
      <!-- Иконка закрытия -->
      ...
      <span class="cursor-pointer" v-if="popupData?.image">
        <picture>
          ...
          <img :src="popupData.image.default" :alt="popupData.altText || 'Image not available'" class="rounded-md"/>
        </picture>
      </span>
      ...
      <div class="info flex flex-col">
        ...
        <p class="text-sm text-gray-400">{{ popupData?.description || 'No Description' }}</p>
      </div>

      <!-- Кнопка -->
      <button class="btn btn-nj-primary mb-2 md:mt-0 w-2/3 mx-auto" v-if="popupData?.buttonText">
...
.popup .icon-close:active, .popup .icon-close:focus{
  rotate: 90deg;
  transition: .3s ease-in-out;
}
